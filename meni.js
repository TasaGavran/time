...
(function () {
  ...
  function renderMenu(items, category) {
    ...
    filtered.forEach(function (item) {
      ...
      el.className = "rounded-2xl border border-espresso/10 bg-white/80 p-4 shadow-md shadow-black/10 backdrop-blur dark:border-white/10 dark:bg-espresso/90";
      el.innerHTML =
  ...
  function loadMenu(currentMenuItems, countEl) {
    fetch(API_BASE_URL + "/menu", { headers: { "Content-Type": "application/json" } })
      .then(function (res) { return res.ok ? res.json() : Promise.reject(new Error()); })
      .then(function (data) {
